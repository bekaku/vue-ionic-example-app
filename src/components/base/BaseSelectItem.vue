...
import { chevronExpandOutline, close } from 'ionicons/icons';
import { computed, defineAsyncComponent, ref } from 'vue';
import BaseButton from './BaseButton.vue';
import BaseIcon from './BaseIcon.vue';
const BaseSelectDialog = defineAsyncComponent(
  () => import('@/components/base/BaseSelectDialog.vue'),
);
const {
  bordered = true,
  multiple = true,
...
<template>
  <ion-item
    ...
    :detail="false"
    button
    :class="{ 'q-ml-md q-mr-lg': withInput, 'bordered': bordered }"
    @click="onClick"
  >
    <template v-if="!multiple && modelValue">
      <!-- <base-avatar v-if="getItemBySelectId(modelValue)?.avatar" slot="start" :src="getItemBySelectId(modelValue)?.avatar || ''" :fetch-image /> -->
      <base-avatar
        v-if="getItemBySelectId(modelValue)?.avatar"
        slot="start"
        v-bind="{
          ...getItemBySelectId(modelValue)?.avatar,
          src: getItemBySelectId(modelValue)?.avatar?.src || '',
        }"
      />
      <BaseIcon
        v-else-if="getItemBySelectId(modelValue)?.icon"
        slot="start"
        v-bind="{
          ...getItemBySelectId(modelValue)?.icon,
          name: getItemBySelectId(modelValue)?.icon?.name || '',
        }"
      />
    </template>
    <ion-label class="ion-text-wrap">
      ...
      >
        <ion-col class="ion-no-padding ion-no-margin">
          <ion-chip
            v-for="(val, orgIndex) in modelValue"
            :key="`org-${val}-${orgIndex}`"
          >
            <base-avatar
              v-if="getItemBySelectId(val)?.avatar"
              v-bind="{
                ...getItemBySelectId(val)?.avatar,
                src: getItemBySelectId(val)?.avatar?.src || '',
                size: 20,
              }"
            />
            <BaseIcon
              v-else-if="getItemBySelectId(val)?.icon"
              v-bind="{
                ...getItemBySelectId(val)?.icon,
                name: getItemBySelectId(val)?.icon?.name || '',
              }"
            />
