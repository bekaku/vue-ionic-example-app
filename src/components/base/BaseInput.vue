  ...
  <IonItem v-bind="$attrs" :lines>
    ...
    <IonLabel>
      <ion-input
        ...
        :type
        :class="{ bordered }"
        @ion-change="$emit('on-change', $event)"
        @ion-blur="onBlur"
        @ion-focus="($event: any) => $emit('on-focus', $event)"
        @ion-input="onInput"
      >
        <IonRow slot="start">
          <slot name="start">
            <!-- <BaseIcon v-if="icon" class="q-mr-sm" :name="icon" :icon-set="iconSet" /> -->
            <BaseIcon v-if="icon != undefined" class="q-mr-sm" v-bind="icon" />
          </slot>
        </IonRow>
        <slot name="label">
          <div slot="label">
