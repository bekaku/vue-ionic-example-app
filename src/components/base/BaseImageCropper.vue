...
import { addOutline, arrowUpOutline, removeOutline, repeatOutline, returnUpBackOutline, returnUpForwardOutline } from 'ionicons/icons';
import { onMounted, onUnmounted, ref, useTemplateRef, watchEffect } from 'vue';
import BaseButton from './BaseButton.vue';
import { biArrowClockwise, biArrowCounterclockwise, biArrowLeftRight, biArrowsCollapse, biZoomIn, biZoomOut } from '@quasar/extras/bootstrap-icons';
...
<template>
  <div class="no-shadow no-border-radius ion-no-margin" :margin="false">
    <base-spinner v-if="loading ||!initFinished" :label="t('base.pleaseWait')" />
    <ion-row class="ion-justify-content-center q-pa-md">
      <cropper-canvas id="cropperSelection" ref="canvasRef"
        :style="{ display: 'block', maxWidth: width, height, overflow: 'hidden' }" class="q-full-width" :disabled
        background @action="onCanvaAction" @actionstart="onCanvaActionstart" @actionmove="onCanvaActionmove"
        @actionend="onCanvaActionend">
        <cropper-image v-if="initFinished" ref="canvasImg" :src="src" alt="Picture" scalable skewable translatable />
        <cropper-shade hidden />
        <cropper-handle action="select" plain />
        <cropper-selection ref="selectionRef" :initial-coverage="0.5" :x="60" :y="60" :width="80" :height="80" movable
          resizable :aspect-ratio="ratio" @change="onSelectionChange">
    ...
    <slot name="preview">
      <ion-row v-if="showPreview && initFinished" class="ion-justify-content-center q-pa-md">
        <img v-if="priviewImage" :src="priviewImage" :style="previewStyle">
      </ion-row>
    </slot>
    <ion-card-content>
      <template v-if="canvasImg">
        <BaseButton size="small" clear flat ion-icon :icon="biZoomIn" icon-set="bootstrap-icons" @click="canvasImg.$zoom(0.1)" />
        <BaseButton size="small" clear flat ion-icon :icon="biZoomOut" icon-set="bootstrap-icons" @click="canvasImg.$zoom(-0.1)" />
        <BaseButton size="small" clear flat ion-icon :icon="biArrowCounterclockwise" icon-set="bootstrap-icons" @click="canvasImg.$rotate('-45deg')" />
        <BaseButton size="small" clear flat ion-icon :icon="biArrowClockwise" icon-set="bootstrap-icons" @click="canvasImg.$rotate('45deg')" />
        <BaseButton size="small" clear flat ion-icon :icon="biArrowLeftRight" icon-set="bootstrap-icons" @click="canvasImg.$scale(-1, 1)" />
        <BaseButton size="small" clear flat ion-icon :icon="biArrowsCollapse" icon-set="bootstrap-icons" @click="canvasImg.$scale(1, -1)" />
      </template>
    </ion-card-content>

    <BaseButton class="q-px-sm" :disabled="submiting" :label="!submiting ? t('base.submit') : t('base.pleaseWait')" full @click="onSubmit" />
  </div>
</template>
