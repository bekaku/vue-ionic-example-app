...
<template>
    <BaseLayout page-title="Image crop" fullscreen show-back-link>
        <BaseCard>
            <ion-card-content>
                <p>Ratio</p>
                <BaseRadioItem v-model="radioSelected" :items="radioOptions" />
                <BaseButton full label="Choose image" @click="showChoosePhoto = true" />
                <BaseButton full label="Delete image" color="danger" @click="onDeleteFile" />
            </ion-card-content>
            <ion-card-content>
                {{ cropedUrl }}
                <template v-if="cropedUrl">
                    ...
                    <p>Fit : scale-down</p>
                    <BaseImage class="q-mb-md" :src="cropedUrl" fit="scale-down" image-bg />
                </template>
            </ion-card-content>
        </BaseCard>

        <BaseImageCropper v-if="showImageCroper && imageFile && imageFile.webPath" v-model="showImageCroper"
            :src="imageFile.webPath" title="Edit image" :ratio="radioSelected" @on-close="onCloseImageCropper"
            @on-okay="onCropImage">
        </BaseImageCropper>

        <BaseChoosePhoto v-if="showChoosePhoto" v-model="showChoosePhoto" :multiple="false"
            @on-pick-picture="onPickPicture" @on-take-picture="onTakePicture" />
    </BaseLayout>
</template>
