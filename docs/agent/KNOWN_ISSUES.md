...
| 16 | `tests/unit/example.spec.ts` imports `@/views/Tab1Page.vue` which does not exist (`src/views/` absent) — vitest suite always fails | VERIFIED pre-existing (Ionic 8 era starter leftover; untouched by Ionic 9 upgrade) | `tests/unit/example.spec.ts:2` | Unit gate red; needs spec rewrite as separate task |
| 17 | `vue-tsc` had 26 pre-existing errors (mostly TS1261 filename-casing); import casings fixed 2026-09-22 leaving 13 non-casing errors (`$t` globals, `AppColor` unions, `IHttpStatus` comparisons, `hammerjs` types). `eslint` has 145 pre-existing errors | VERIFIED (before/after error-set comparison) | `npx vue-tsc --noEmit`; `pnpm lint` | Static gates still red for reasons unrelated to Ionic version |
