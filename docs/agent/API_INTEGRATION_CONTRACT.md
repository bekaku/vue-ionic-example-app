...
`/auth/login` (`useApi.ts` › `refreshToken`, `executeFetch`). Details: `skills/mobile/API.md` §1.
Migrated from `src/plugins/axios.ts` on 2026-09-24
(`tasks/002-migrate-axios-to-useapi.md`).

## Verified call-sites

| Method | Path | Caller | Class |
| ------ | ---- | ------ | ----- |
| GET | `/api/appUser/currentUserData` | `authenStore.ts` › `initialAuthDataProcess` | VERIFIED_FRONTEND_USAGE |
