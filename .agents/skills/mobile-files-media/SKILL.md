...
description: >
  ...
  file preview, sharing, or device storage.
---
...
- `src/composables/useFileSystem.ts`, `src/composables/useFileUpload.ts`
...
3. Upload: 1 MB chunks, 3 retries, `uploadChunkApi`/`mergeChunkApi`; chunked ≠
   resumable without server proof.
4. Validate paths, MIME, filenames; respect scoped storage / sandbox; request
   minimal permissions with `isWeb()` guards.

## Implementation workflow

1. Trace capture/pick → convert → store/upload → cleanup chain.
